...
18. **Thicken `hanage.app` before resubmitting to AdSense** (Phase D item 8). Detailed content policy, concrete article candidates, and the 2026-09-22 small-browser-game-site research are recorded in [`docs/ADSENSE_CONTENT_PLAN.md`](./ADSENSE_CONTENT_PLAN.md). The current decision is to **skip strategy/攻略 articles and generic SEO writing**, keep the copy concise, and center the expansion on first-hand development notes plus small improvements to the game pages, `/updates/`, and `/about/`. The gap is original writing that lives on the reviewed domain, not page count, and not anything on the game subdomains. In rough order of weight:
    ...
    - ~~**Strategy writing**, one level past the how-to-play pages.~~ **Dropped on 2026-09-22.** Do not add攻略記事 just to thicken the site; use first-hand development notes instead.
