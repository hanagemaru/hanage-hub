...
const STAGES=[
 ...
 {name:"HILL CREST",short:"HILL",desc:"上り→頂上→下り。大型車が上りで失速",n:19,laps:3,jamLimit:8.5,hill:true,trucks:1},
 {name:"HAIRPIN + TUNNEL",short:"CURVE",desc:"実形状の減速コーナー＋トンネル",n:19,laps:3,jamLimit:8.5,curve:true,tunnel:true},
 {name:"RUSH HOUR",short:"RUSH",desc:"坂・減速コーナー・トンネル・合流",n:20,laps:3,jamLimit:8.5,hill:true,curve:true,tunnel:true,trucks:1,events:[{at:.4,cars:[false]},{at:1.2,cars:[false]},{at:2.0,cars:[false]}]}
];
...
function reset(i=stageIndex){
  ...
  player=cars.find(c=>c.player);started=false;running=true;paused=false;lastTs=0;totalDist=0;noticeTimer=0;autoBrake=false;failClock=0;resultShown=false;
...
function update(realDt){
  ...
  const st=jamStats();failClock=st.best>=cfg().jamLimit?Math.min(.8,failClock+realDt):Math.max(0,failClock-realDt*1.2);
...
function finish(ok){
  ...
  overlay.innerHTML=ok
    ?`<div class="overlayCard"><h2 class="good">FINISH</h2><p>${cfg().name}</p><p>JAM ${Math.round(st.jamPct)}%</p><button id="nextBtn" class="mainBtn">${last?"STAGE SELECT":"NEXT STAGE"}</button></div>`
    :`<div class="overlayCard"><h2 class="bad">GRIDLOCK</h2><p>JAM LEVEL 100%</p><button id="retryBtn" class="mainBtn">RETRY</button></div>`;
...
function drawCar(c){
  const p=roadPoint(c.s),sc=sev(c.jamLevel);ctx.save();ctx.translate(p.x,p.y);ctx.rotate(p.a);
...
function render(pre){
  ...
  if(!started){stateText.textContent="PRESS A PEDAL";phaseTag.textContent="READY";centerMain.textContent=cfg().name;centerSub.textContent=cfg().seed?"前に空間を作って波を吸収":"3周走り切る"}
...
function showStageSelect(){
  ...
  const g=$("stageGrid");STAGES.forEach((s,i)=>{const b=document.createElement("button");b.innerHTML=`${i+1}. ${s.name}<small>${s.desc}</small>`;b.onclick=()=>reset(i);g.appendChild(b)});
}
