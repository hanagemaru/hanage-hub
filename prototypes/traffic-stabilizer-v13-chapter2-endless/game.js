...
function cfg(){return gameMode==="endless"?ENDLESS:STAGES[stageIndex]}
...
function reset(i=stageIndex,nextMode=gameMode){
  gameMode=nextMode;if(gameMode==="campaign")stageIndex=i;const s=cfg();buildTrack();cars=[];rampCars=[];eventIndex=0;nextId=100;
  if(s.seed){
  ...
  stageTag.textContent=gameMode==="endless"?"ENDLESS FLOW":`STAGE ${stageIndex+1}/${STAGES.length} · ${s.short}`;sub.textContent=s.desc;render();
}
...
function showModeSelect(){
  ...
  overlay.innerHTML=`<div class="overlayCard"><h2>SELECT MODE</h2><div class="modeMenu"><button id="ch1" class="modeBtn">CHAPTER 1<small>STAGE 1–7 · BASIC</small></button><button id="ch2" class="modeBtn">CHAPTER 2<small>STAGE 8–14 · ADVANCED</small></button><button id="endless" class="modeBtn endless">ENDLESS FLOW<small>BEST ${best.toFixed(1)} LAPS · GRIDLOCKまで走る</small></button></div></div>`;
  overlay.classList.remove("hidden");
  $("ch1").onclick=()=>showChapter(1);$("ch2").onclick=()=>showChapter(2);$("endless").onclick=()=>reset(0,"endless");
}
function showChapter(chapter){
  ...
  overlay.classList.remove("hidden");
  const g=$("stageGrid");
  STAGES.slice(start,end).forEach((s,j)=>{const idx=start+j,b=document.createElement("button");b.innerHTML=`${idx+1}. ${s.name}${s.tutorial?" · TUTORIAL":""}<small>${s.desc}</small>`;b.onclick=()=>reset(idx,"campaign");g.appendChild(b)});
