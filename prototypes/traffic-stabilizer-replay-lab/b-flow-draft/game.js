...
const DRAFT_BASE={name:"FLOW DRAFT",short:"DRAFT",desc:"2周ごとに次の負荷を選ぶ",n:17,laps:10,jamLimit:9.2,draft:true,pace:REGULAR_SIM_SPEED,events:[]};
let DRAFT={...DRAFT_BASE};

let stageIndex=0,gameMode="campaign",cars=[],rampCars=[],player=null,started=false,running=false,paused=false,lastTs=0,totalDist=0,nextId=100,eventIndex=0,noticeTimer=0,autoBrake=false,failClock=0,resultShown=false,elapsedReal=0,maxJamPct=0,tutorialStep=0,attemptCounted=false,draftNext=2,draftMult=1,draftScore=0,draftChoices=0,draftPending=false;
let trackRaw=[],trackCum=[],trackTotal=0;
const input={gas:false,brake:false};

function cfg(){return gameMode==="endless"?ENDLESS:gameMode==="daily"?DAILY:gameMode==="draft"?DRAFT:STAGES[stageIndex]}
...
function reset(i=stageIndex,nextMode=gameMode){
  gameMode=nextMode;if(gameMode==="campaign")stageIndex=i;if(gameMode==="daily")DAILY=buildDailyConfig();if(gameMode==="draft")DRAFT={...DRAFT_BASE,events:[]};const s=cfg();buildTrack();cars=[];rampCars=[];eventIndex=0;nextId=100;
  if(s.seed){
  ...
  player=cars.find(c=>c.player);started=false;running=true;paused=false;lastTs=0;totalDist=0;noticeTimer=0;autoBrake=false;failClock=0;resultShown=false;elapsedReal=0;maxJamPct=0;tutorialStep=0;attemptCounted=false;draftNext=2;draftMult=1;draftScore=0;draftChoices=0;draftPending=false;
  ...
  stageTag.textContent=gameMode==="endless"?"ENDLESS FLOW":gameMode==="daily"?"DAILY CHALLENGE":gameMode==="draft"?"FLOW DRAFT":`STAGE ${stageIndex+1}/${STAGES.length} · ${s.short}`;sub.textContent=s.desc;render();
}
...
function update(realDt){
  ...
  if(cfg().draft){
    ...
    draftScore+=distanceStep*(.50+.50*speedQuality)*(.45+.55*smoothQuality)*draftMult*2;
  }
  if(cfg().tutorial){
  ...
  if(cfg().draft&&totalDist/L>=draftNext&&draftNext<cfg().laps){showDraftChoice();return}
...
function getDraftBest(){try{return Number(localStorage.getItem("trafficStabilizerDraftBest")||0)}catch(e){return 0}}
function setDraftBest(v){try{localStorage.setItem("trafficStabilizerDraftBest",String(v))}catch(e){}}
function draftCards(){
  const cards=[
    {id:"surge",name:"SURGE +2",desc:"普通車2台が合流",mult:1.12,apply:()=>queueRamp([false,false],"SURGE +2")},
    {id:"truck",name:"TRUCK +1",desc:"大型車1台が合流",mult:1.18,apply:()=>queueRamp([true],"TRUCK +1")},
    {id:"works",name:"ROAD WORKS",desc:"工事区間を有効化",mult:1.15,active:()=>DRAFT.works,apply:()=>{DRAFT.works=true}},
    {id:"hill",name:"HILL",desc:"上り→頂上→下りを有効化",mult:1.15,active:()=>DRAFT.hill,apply:()=>{DRAFT.hill=true}},
    {id:"curve",name:"HAIRPIN",desc:"急カーブを有効化",mult:1.15,active:()=>DRAFT.curve,apply:()=>{DRAFT.curve=true}},
    {id:"tunnel",name:"TUNNEL",desc:"半透明トンネルを有効化",mult:1.10,active:()=>DRAFT.tunnel,apply:()=>{DRAFT.tunnel=true}}
  ];
  return cards.filter(c=>!c.active||!c.active());
}
function showDraftChoice(){
  if(draftPending)return;draftPending=true;running=false;
  const pool=draftCards(),seed=Math.floor(totalDist/L*997)+draftChoices*37;
  let a=pool[seed%pool.length],b=pool[(seed*3+1)%pool.length];
  if(a===b)b=pool[(pool.indexOf(a)+1)%pool.length];
  overlay.innerHTML=`<div class="overlayCard"><h2>CHOOSE NEXT PRESSURE</h2><p>LAP ${Math.floor(totalDist/L)+1} · SCORE x${draftMult.toFixed(2)}</p><div class="draftGrid"><button id="draftA" class="draftCard"><b>${a.name}</b><small>${a.desc}</small><div class="draftMult">×${a.mult.toFixed(2)}</div></button><button id="draftB" class="draftCard"><b>${b.name}</b><small>${b.desc}</small><div class="draftMult">×${b.mult.toFixed(2)}</div></button></div></div>`;
  overlay.classList.remove("hidden");
  const choose=card=>{card.apply();draftMult*=card.mult;draftChoices++;draftNext+=2;draftPending=false;overlay.classList.add("hidden");running=true;showNotice(card.name)};
  $("draftA").onclick=()=>choose(a);$("draftB").onclick=()=>choose(b);
}
...
function finish(ok){
  ...
  if(cfg().draft){
    const score=Math.round(draftScore),oldBest=getDraftBest(),isNew=score>oldBest,best=Math.max(score,oldBest);if(isNew)setDraftBest(best);
...
function showModeSelect(){
  running=false;input.gas=input.brake=false;brakeBtn.classList.remove("held");gasBtn.classList.remove("held");
  const best=getEndlessBest(),draftBest=getDraftBest(),daily=buildDailyConfig(),dr=getDailyRecord(daily.dateKey),records=getStageRecords(),next=firstUnclearedStage();
  ...
  overlay.innerHTML=`<div class="overlayCard"><h2>SELECT MODE</h2><div class="modeMenu"><button id="continue" class="modeBtn wide">${allClear?"CAMPAIGN COMPLETE · REPLAY":"CONTINUE CAMPAIGN"}<small>${allClear?"STAGE 28 · FINAL WAVE":`STAGE ${next+1} · ${STAGES[next].name}`}</small></button><button id="ch1" class="modeBtn">CHAPTER 1<small>${count(1)}/7 · BASIC</small></button><button id="ch2" class="modeBtn">CHAPTER 2<small>${count(2)}/7 · ADVANCED</small></button><button id="ch3" class="modeBtn">CHAPTER 3<small>${count(3)}/7 · WORKS</small></button><button id="ch4" class="modeBtn">CHAPTER 4<small>${count(4)}/7 · MASTER</small></button><button id="daily" class="modeBtn daily">DAILY<small>${daily.dateKey} · ${dr.cleared?"CLEAR":"NEW"}</small></button><button id="draft" class="modeBtn endless">FLOW DRAFT<small>BEST ${draftBest.toLocaleString()} · CHOOSE RISK</small></button><button id="endless" class="modeBtn endless">ENDLESS<small>BEST ${best.toFixed(1)} LAPS</small></button><button id="settings" class="modeBtn wide">SETTINGS<small>PRIVACY · TERMS · LOCAL DATA</small></button></div></div>`;
  overlay.classList.remove("hidden");
  $("continue").onclick=()=>reset(allClear?STAGES.length-1:next,"campaign");
  $("ch1").onclick=()=>showChapter(1);$("ch2").onclick=()=>showChapter(2);$("ch3").onclick=()=>showChapter(3);$("ch4").onclick=()=>showChapter(4);$("daily").onclick=()=>reset(0,"daily");$("draft").onclick=()=>reset(0,"draft");$("endless").onclick=()=>reset(0,"endless");$("settings").onclick=showSettings;
}
...
reset(0,"draft");requestAnimationFrame(frame);
})();