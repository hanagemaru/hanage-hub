...
function cfg(){return gameMode==="endless"?ENDLESS:gameMode==="daily"?DAILY:gameMode==="contract"?CONTRACT:STAGES[stageIndex]}
...
function reset(i=stageIndex,nextMode=gameMode){
  ...
  player=cars.find(c=>c.player);started=false;running=true;paused=false;lastTs=0;totalDist=0;noticeTimer=0;autoBrake=false;failClock=0;resultShown=false;elapsedReal=0;maxJamPct=0;tutorialStep=0;attemptCounted=false;autoBrakeTime=0;
  ...
  stageTag.textContent=gameMode==="endless"?"ENDLESS FLOW":gameMode==="daily"?"DAILY CHALLENGE":gameMode==="contract"?"CONTRACT RUN":`STAGE ${stageIndex+1}/${STAGES.length} · ${s.short}`;sub.textContent=s.desc;render();
}
...
function update(realDt){
  ...
  if(cfg().contract&&autoBrake)autoBrakeTime+=realDt;
  if(cfg().tutorial){
...
function getContractProgress(){try{return JSON.parse(localStorage.getItem("trafficStabilizerContracts")||"{}")}catch(e){return{}}}
...
function contractActual(){
  ...
  if(contractSelected.id==="smooth")return `MAX JAM ${Math.round(maxJamPct)}%`;
  if(contractSelected.id==="fast")return `TIME ${formatTime(elapsedReal)}`;
  return `AUTO BRAKE ${autoBrakeTime.toFixed(1)}s`;
}
...
function finish(ok){
  ...
  if(cfg().contract){
    const passed=ok&&contractPassed();if(passed)saveContractDone(contractSelected.id);
...
function showContractSelect(){
  running=false;input.gas=input.brake=false;brakeBtn.classList.remove("held");gasBtn.classList.remove("held");
  const p=getContractProgress(),done=CONTRACTS.filter(c=>p[c.id]).length;
  overlay.innerHTML=`<div class="overlayCard"><h2>CONTRACT RUN</h2><p>${done}/3 CLEARED · same road, different definition of “good”</p><div id="contractGrid" class="contractGrid"></div><button id="backContracts" class="backBtn">← MODE SELECT</button></div>`;
  overlay.classList.remove("hidden");
  const grid=$("contractGrid");
  CONTRACTS.forEach(c=>{const b=document.createElement("button");b.className="contractBtn";b.innerHTML=`<b>${c.name}</b><small>${c.desc}</small>${p[c.id]?'<div class="contractDone">✓ CLEARED</div>':""}`;b.onclick=()=>{contractSelected=c;reset(0,"contract")};grid.appendChild(b)});
  $("backContracts").onclick=showModeSelect;
}
function showModeSelect(){
  running=false;input.gas=input.brake=false;brakeBtn.classList.remove("held");gasBtn.classList.remove("held");
  const best=getEndlessBest(),contractProgress=getContractProgress(),contractDone=CONTRACTS.filter(c=>contractProgress[c.id]).length,daily=buildDailyConfig(),dr=getDailyRecord(daily.dateKey),records=getStageRecords(),next=firstUnclearedStage();
  ...
  overlay.innerHTML=`<div class="overlayCard"><h2>SELECT MODE</h2><div class="modeMenu"><button id="continue" class="modeBtn wide">${allClear?"CAMPAIGN COMPLETE · REPLAY":"CONTINUE CAMPAIGN"}<small>${allClear?"STAGE 28 · FINAL WAVE":`STAGE ${next+1} · ${STAGES[next].name}`}</small></button><button id="ch1" class="modeBtn">CHAPTER 1<small>${count(1)}/7 · BASIC</small></button><button id="ch2" class="modeBtn">CHAPTER 2<small>${count(2)}/7 · ADVANCED</small></button><button id="ch3" class="modeBtn">CHAPTER 3<small>${count(3)}/7 · WORKS</small></button><button id="ch4" class="modeBtn">CHAPTER 4<small>${count(4)}/7 · MASTER</small></button><button id="daily" class="modeBtn daily">DAILY<small>${daily.dateKey} · ${dr.cleared?"CLEAR":"NEW"}</small></button><button id="contract" class="modeBtn daily">CONTRACT RUN<small>${contractDone}/3 CLEAR · CHOOSE GOAL</small></button><button id="endless" class="modeBtn endless">ENDLESS<small>BEST ${best.toFixed(1)} LAPS</small></button><button id="settings" class="modeBtn wide">SETTINGS<small>PRIVACY · TERMS · LOCAL DATA</small></button></div></div>`;
  overlay.classList.remove("hidden");
  $("continue").onclick=()=>reset(allClear?STAGES.length-1:next,"campaign");
  $("ch1").onclick=()=>showChapter(1);$("ch2").onclick=()=>showChapter(2);$("ch3").onclick=()=>showChapter(3);$("ch4").onclick=()=>showChapter(4);$("daily").onclick=()=>reset(0,"daily");$("contract").onclick=showContractSelect;$("endless").onclick=()=>reset(0,"endless");$("settings").onclick=showSettings;
}
...
reset(0,"campaign");showContractSelect();requestAnimationFrame(frame);
})();