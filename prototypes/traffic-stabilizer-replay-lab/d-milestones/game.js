...
function dailyClearCount(){
  let n=0;
  try{
    for(let i=0;i<localStorage.length;i++){
      const k=localStorage.key(i);
      if(k&&k.startsWith("trafficStabilizerDaily:")){
        try{if(JSON.parse(localStorage.getItem(k)||"{}").cleared)n++}catch(e){}
      }
    }
  }catch(e){}
  return n;
...
function milestoneData(){
  const r=getStageRecords(),cleared=Object.keys(r).filter(k=>r[k]?.cleared&&Number(k)<STAGES.length).map(Number),vals=Object.values(r).filter(x=>x?.cleared);
  const chapter=n=>{let c=0;for(let i=(n-1)*7;i<n*7;i++)if(r[String(i)]?.cleared)c++;return c};
  const minJam=vals.length?Math.min(...vals.map(x=>x.bestJam??999)):999,minTime=vals.length?Math.min(...vals.map(x=>x.bestTime??999)):999,endless=getEndlessBest(),dailies=dailyClearCount();
  return[
    {id:"first",name:"FIRST FLOW",desc:"Campaignを1面クリア",done:cleared.length>=1},
    {id:"smooth60",name:"SMOOTH 60",desc:"MAX JAM 60%以下",done:minJam<=60},
    {id:"smooth40",name:"SMOOTH 40",desc:"MAX JAM 40%以下",done:minJam<=40},
    {id:"quick80",name:"QUICK FLOW",desc:"80秒以内でクリア",done:minTime<=80},
    {id:"c1",name:"BASIC COMPLETE",desc:"Chapter 1を全クリア",done:chapter(1)>=7},
    {id:"c2",name:"ADVANCED COMPLETE",desc:"Chapter 2を全クリア",done:chapter(2)>=7},
    {id:"c3",name:"WORKS COMPLETE",desc:"Chapter 3を全クリア",done:chapter(3)>=7},
    {id:"final",name:"FINAL WAVE",desc:"Stage 28をクリア",done:!!r["27"]?.cleared},
    {id:"daily1",name:"DAILY DRIVER",desc:"Dailyを1日クリア",done:dailies>=1},
    {id:"daily3",name:"DAILY HABIT",desc:"Dailyを3日クリア",done:dailies>=3},
    {id:"end10",name:"LONG FLOW",desc:"Endless 10 laps",done:endless>=10},
    {id:"all",name:"FLOW MASTER",desc:"Campaign 28面クリア",done:cleared.length>=28}
  ];
}
...
function finish(ok){
  ...
  const bestMsg=(newTime||newJam)?`<div class="bestNote new">NEW BEST${newTime&&newJam?" · TIME + JAM":newTime?" · TIME":" · JAM"}</div>`:"";const nextMilestone=milestoneData().find(x=>!x.done);const milestoneHint=nextMilestone?`<div class="bestNote">NEXT: ${nextMilestone.name} · ${nextMilestone.desc}</div>`:"";
  overlay.innerHTML=ok
    ?`<div class="overlayCard"><h2 class="good">FINISH</h2><p>${cfg().name}</p><div class="resultStats"><div class="resultStat"><span>MAX JAM</span><b>${Math.round(maxJamPct)}%</b></div><div class="resultStat"><span>TIME</span><b>${formatTime(elapsedReal)}</b></div></div>${bestMsg}${milestoneHint}<button id="nextBtn" class="mainBtn">${last?"MODE SELECT":"NEXT STAGE"}</button></div>`
...
function showMilestones(){
  ...
  const ms=milestoneData(),done=ms.filter(x=>x.done).length,next=ms.find(x=>!x.done);
  overlay.innerHTML=`<div class="overlayCard"><h2>MILESTONES</h2><div class="milestoneSummary">${done}/${ms.length} COMPLETE</div><div class="nextGoal">${next?"NEXT · "+next.name+" — "+next.desc:"ALL MILESTONES COMPLETE"}</div><div id="milestoneGrid" class="milestoneGrid"></div><button id="backMilestones" class="backBtn">← MODE SELECT</button></div>`;
  overlay.classList.remove("hidden");
  const grid=$("milestoneGrid");
  ms.forEach(m=>{const d=document.createElement("div");d.className="milestone "+(m.done?"done":"");d.innerHTML=`<span class="mark">${m.done?"✓":"○"}</span><b>${m.name}</b><small>${m.desc}</small>`;grid.appendChild(d)});
  $("backMilestones").onclick=showModeSelect;
}
...
function showModeSelect(){
  running=false;input.gas=input.brake=false;brakeBtn.classList.remove("held");gasBtn.classList.remove("held");
  const best=getEndlessBest(),milestones=milestoneData(),milestoneDone=milestones.filter(x=>x.done).length,daily=buildDailyConfig(),dr=getDailyRecord(daily.dateKey),records=getStageRecords(),next=firstUnclearedStage();
  ...
  overlay.innerHTML=`<div class="overlayCard"><h2>SELECT MODE</h2><div class="modeMenu"><button id="continue" class="modeBtn wide">${allClear?"CAMPAIGN COMPLETE · REPLAY":"CONTINUE CAMPAIGN"}<small>${allClear?"STAGE 28 · FINAL WAVE":`STAGE ${next+1} · ${STAGES[next].name}`}</small></button><button id="ch1" class="modeBtn">CHAPTER 1<small>${count(1)}/7 · BASIC</small></button><button id="ch2" class="modeBtn">CHAPTER 2<small>${count(2)}/7 · ADVANCED</small></button><button id="ch3" class="modeBtn">CHAPTER 3<small>${count(3)}/7 · WORKS</small></button><button id="ch4" class="modeBtn">CHAPTER 4<small>${count(4)}/7 · MASTER</small></button><button id="daily" class="modeBtn daily">DAILY<small>${daily.dateKey} · ${dr.cleared?"CLEAR":"NEW"}</small></button><button id="milestones" class="modeBtn daily">MILESTONES<small>${milestoneDone}/${milestones.length} · LONG GOALS</small></button><button id="endless" class="modeBtn endless">ENDLESS<small>BEST ${best.toFixed(1)} LAPS</small></button><button id="settings" class="modeBtn wide">SETTINGS<small>PRIVACY · TERMS · LOCAL DATA</small></button></div></div>`;
  overlay.classList.remove("hidden");
  $("continue").onclick=()=>reset(allClear?STAGES.length-1:next,"campaign");
  $("ch1").onclick=()=>showChapter(1);$("ch2").onclick=()=>showChapter(2);$("ch3").onclick=()=>showChapter(3);$("ch4").onclick=()=>showChapter(4);$("daily").onclick=()=>reset(0,"daily");$("milestones").onclick=showMilestones;$("endless").onclick=()=>reset(0,"endless");$("settings").onclick=showSettings;
}
...
reset(0,"campaign");showMilestones();requestAnimationFrame(frame);
})();