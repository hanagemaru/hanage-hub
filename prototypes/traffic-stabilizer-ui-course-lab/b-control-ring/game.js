...
function getStageRecords(){
  try{return JSON.parse(localStorage.getItem("trafficStabilizerLabBStageRecords")||"{}")}catch(e){return{}}
}
...
function saveStageRecord(index,time,jam){
  try{
    ...
    localStorage.setItem("trafficStabilizerLabBStageRecords",JSON.stringify(all));
  }catch(e){}
}
function getDailyRecord(key=japanDayKey()){
  try{return JSON.parse(localStorage.getItem("trafficStabilizerLabBDaily:"+key)||"{}")}catch(e){return{}}
}
function noteDailyAttempt(key){
  try{const r=getDailyRecord(key);r.attempts=(r.attempts||0)+1;localStorage.setItem("trafficStabilizerLabBDaily:"+key,JSON.stringify(r))}catch(e){}
}
function saveDailyRecord(key,time,jam){
  try{
    ...
    localStorage.setItem("trafficStabilizerLabBDaily:"+key,JSON.stringify(r));
  }catch(e){}
}
...
let JOURNEY=null;
const JOURNEY_KEY="trafficStabilizerLabBJourneyV1";
...
function getEndlessBest(){
  try{return Number(localStorage.getItem("trafficStabilizerLabBEndlessBest")||0)}catch(e){return 0}
}
function setEndlessBest(v){
  try{localStorage.setItem("trafficStabilizerLabBEndlessBest",String(v))}catch(e){}
}
...
function render(pre){
  ...
  progressFill.style.width=`${started?frac*100:0}%`;dangerFill.style.width=`${st.jamPct}%`;dangerFill.style.background=col;canvas.parentElement.style.setProperty("--jam-ui",col);
