...
const L=360,CAR=4.5,V0=27.8,S0=2,DELTA=4,R=220,CX=360,CY=260,REGULAR_SIM_SPEED=3.75,TUTORIAL_SIM_SPEED=2.5,PLAYER_MAX=52/3.6;
...
function getStageRecords(){
  try{return JSON.parse(localStorage.getItem("trafficStabilizerLabCStageRecords")||"{}")}catch(e){return{}}
}
...
function saveStageRecord(index,time,jam){
  try{
    ...
    localStorage.setItem("trafficStabilizerLabCStageRecords",JSON.stringify(all));
  }catch(e){}
}
function getDailyRecord(key=japanDayKey()){
  try{return JSON.parse(localStorage.getItem("trafficStabilizerLabCDaily:"+key)||"{}")}catch(e){return{}}
}
function noteDailyAttempt(key){
  try{const r=getDailyRecord(key);r.attempts=(r.attempts||0)+1;localStorage.setItem("trafficStabilizerLabCDaily:"+key,JSON.stringify(r))}catch(e){}
}
function saveDailyRecord(key,time,jam){
  try{
    ...
    localStorage.setItem("trafficStabilizerLabCDaily:"+key,JSON.stringify(r));
  }catch(e){}
}
...
let JOURNEY=null;
const JOURNEY_KEY="trafficStabilizerLabCJourneyV1";
...
function rawTrackPoint(t){
  const s=t*L,a=-Math.PI/2+2*Math.PI*t,ca=Math.cos(a),sa=Math.sin(a);
  const sx=Math.sign(ca||1)*Math.pow(Math.abs(ca),.58),sy=Math.sign(sa||1)*Math.pow(Math.abs(sa),.58);
  let x=CX+270*sx,y=CY+168*sy;
  if(cfg().curve){
    const u=zoneU(s,CURVE);
    if(u>=0){
      const env=Math.sin(Math.PI*u);
      x+=46*env;
      y+=24*env*Math.sin(2*Math.PI*u);
    }
  }
  if(cfg().hill){
...
function rampGeom(){
  ...
  return{p2:{x:ep.x,y:ep.y},p1:{x:ep.x-tx*55+nx*22,y:ep.y-ty*55+ny*22},p0:{x:ep.x-tx*105+nx*76,y:ep.y-ty*105+ny*76}};
}
...
function getEndlessBest(){
  try{return Number(localStorage.getItem("trafficStabilizerLabCEndlessBest")||0)}catch(e){return 0}
}
function setEndlessBest(v){
  try{localStorage.setItem("trafficStabilizerLabCEndlessBest",String(v))}catch(e){}
}
...
function drawZoneLabel(s,lateral,text,bg="#fffdfa"){
  const p=roadPoint(s,lateral);ctx.save();ctx.font="700 12px system-ui";ctx.textAlign="center";ctx.textBaseline="middle";
  const w=Math.max(38,ctx.measureText(text).width+12);ctx.fillStyle=bg;ctx.strokeStyle="rgba(30,30,30,.35)";ctx.lineWidth=1;
  ctx.fillRect(p.x-w/2,p.y-10,w,20);ctx.strokeRect(p.x-w/2,p.y-10,w,20);ctx.fillStyle="#171717";ctx.fillText(text,p.x,p.y+.5);ctx.restore();
}
function drawWorkFeatures(){
  if(!cfg().works)return;
  strokeTrack("rgba(224,132,36,.34)",66,0,WORKS[0],WORKS[1]);
  drawZoneLabel((WORKS[0]+WORKS[1])/2,54,"WORK","#f7d3aa");
...
function drawHillFeatures(){
  ...
  const crest=roadPoint(89,46);ctx.beginPath();ctx.arc(crest.x,crest.y,7,0,Math.PI*2);ctx.fillStyle="#f6e3a3";ctx.fill();ctx.strokeStyle="#7b5e2e";ctx.lineWidth=2;ctx.stroke();drawZoneLabel(88,62,"CREST","#f6e3a3");
}
function drawCurveFeatures(){
  if(!cfg().curve)return;
  strokeTrack("rgba(111,79,145,.30)",66,0,CURVE[0],CURVE[1]);
  strokeTrack("#f0c84b",8,43,CURVE[0],CURVE[1]);
  for(const s of [164,176,188,200,212,222])drawChevron(s,43,"#202020",1);drawZoneLabel(190,58,"CURVE","#eadcf2");
}
function drawTunnel(){
  if(!cfg().tunnel)return;
  strokeTrack("rgba(19,24,29,.94)",68,0,TUNNEL[0],TUNNEL[1]);
  for(const edge of [TUNNEL[0],TUNNEL[1]])drawCrossBar(edge,"#c7d0d6",9,46);drawZoneLabel((TUNNEL[0]+TUNNEL[1])/2,55,"TUNNEL","#c7d0d6");
...
function drawRoad(){
  ctx.fillStyle="#cbd1c6";ctx.fillRect(0,0,canvas.width,canvas.height);
