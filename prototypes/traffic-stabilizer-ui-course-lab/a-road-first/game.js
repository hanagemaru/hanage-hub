...
function getStageRecords(){
  try{return JSON.parse(localStorage.getItem("trafficStabilizerLabAStageRecords")||"{}")}catch(e){return{}}
}
...
function saveStageRecord(index,time,jam){
  try{
    ...
    localStorage.setItem("trafficStabilizerLabAStageRecords",JSON.stringify(all));
  }catch(e){}
}
function getDailyRecord(key=japanDayKey()){
  try{return JSON.parse(localStorage.getItem("trafficStabilizerLabADaily:"+key)||"{}")}catch(e){return{}}
}
function noteDailyAttempt(key){
  try{const r=getDailyRecord(key);r.attempts=(r.attempts||0)+1;localStorage.setItem("trafficStabilizerLabADaily:"+key,JSON.stringify(r))}catch(e){}
}
function saveDailyRecord(key,time,jam){
  try{
    ...
    localStorage.setItem("trafficStabilizerLabADaily:"+key,JSON.stringify(r));
  }catch(e){}
}
...
let JOURNEY=null;
const JOURNEY_KEY="trafficStabilizerLabAJourneyV1";
...
function getEndlessBest(){
  try{return Number(localStorage.getItem("trafficStabilizerLabAEndlessBest")||0)}catch(e){return 0}
}
function setEndlessBest(v){
  try{localStorage.setItem("trafficStabilizerLabAEndlessBest",String(v))}catch(e){}
}
