...
function getStageRecords(){
  try{return JSON.parse(localStorage.getItem("trafficStabilizerLabDStageRecords")||"{}")}catch(e){return{}}
}
...
function saveStageRecord(index,time,jam){
  try{
    ...
    localStorage.setItem("trafficStabilizerLabDStageRecords",JSON.stringify(all));
  }catch(e){}
}
function getDailyRecord(key=japanDayKey()){
  try{return JSON.parse(localStorage.getItem("trafficStabilizerLabDDaily:"+key)||"{}")}catch(e){return{}}
}
function noteDailyAttempt(key){
  try{const r=getDailyRecord(key);r.attempts=(r.attempts||0)+1;localStorage.setItem("trafficStabilizerLabDDaily:"+key,JSON.stringify(r))}catch(e){}
}
function saveDailyRecord(key,time,jam){
  try{
    ...
    localStorage.setItem("trafficStabilizerLabDDaily:"+key,JSON.stringify(r));
  }catch(e){}
}
...
let JOURNEY=null;
const JOURNEY_KEY="trafficStabilizerLabDJourneyV1";
...
function rawTrackPoint(t){
  ...
  const sx=Math.sign(ca||1)*Math.pow(Math.abs(ca),.68),sy=Math.sign(sa||1)*Math.pow(Math.abs(sa),.72);
  let x=CX+220*sx+28*Math.cos(2*a)+12*Math.sin(3*a);
  let y=CY+188*sy+24*Math.sin(2*a)-10*Math.cos(3*a);
  if(cfg().curve){
    const u=zoneU(s,CURVE);
    if(u>=0){
      const env=Math.sin(Math.PI*u);
      x-=34*env;
      y+=28*env*Math.sin(2*Math.PI*u);
    }
  }
  if(cfg().hill){
...
function rampGeom(){
  ...
  return{p2:{x:ep.x,y:ep.y},p1:{x:ep.x-tx*62+nx*25,y:ep.y-ty*62+ny*25},p0:{x:ep.x-tx*118+nx*84,y:ep.y-ty*118+ny*84}};
}
...
function getEndlessBest(){
  try{return Number(localStorage.getItem("trafficStabilizerLabDEndlessBest")||0)}catch(e){return 0}
}
function setEndlessBest(v){
  try{localStorage.setItem("trafficStabilizerLabDEndlessBest",String(v))}catch(e){}
}
...
function drawZoneLabel(s,lateral,text,bg="#fffdfa"){
  const p=roadPoint(s,lateral);ctx.save();ctx.font="700 11px system-ui";ctx.textAlign="center";ctx.textBaseline="middle";
  const w=Math.max(36,ctx.measureText(text).width+10);ctx.fillStyle=bg;ctx.strokeStyle="rgba(30,30,30,.32)";ctx.lineWidth=1;
  ctx.fillRect(p.x-w/2,p.y-9,w,18);ctx.strokeRect(p.x-w/2,p.y-9,w,18);ctx.fillStyle="#171717";ctx.fillText(text,p.x,p.y+.5);ctx.restore();
}
function drawWorkFeatures(){
  if(!cfg().works)return;
  strokeTrack("rgba(224,132,36,.34)",66,0,WORKS[0],WORKS[1]);
  drawZoneLabel((WORKS[0]+WORKS[1])/2,52,"WORK","#f7d3aa");
...
function drawHillFeatures(){
  ...
  const crest=roadPoint(89,46);ctx.beginPath();ctx.arc(crest.x,crest.y,7,0,Math.PI*2);ctx.fillStyle="#f6e3a3";ctx.fill();ctx.strokeStyle="#7b5e2e";ctx.lineWidth=2;ctx.stroke();drawZoneLabel(88,58,"CREST","#f6e3a3");
}
function drawCurveFeatures(){
  if(!cfg().curve)return;
  strokeTrack("rgba(111,79,145,.30)",66,0,CURVE[0],CURVE[1]);
  strokeTrack("#f0c84b",8,43,CURVE[0],CURVE[1]);
  for(const s of [164,176,188,200,212,222])drawChevron(s,43,"#202020",1);drawZoneLabel(190,56,"CURVE","#eadcf2");
}
function drawTunnel(){
  if(!cfg().tunnel)return;
  strokeTrack("rgba(19,24,29,.94)",68,0,TUNNEL[0],TUNNEL[1]);
  for(const edge of [TUNNEL[0],TUNNEL[1]])drawCrossBar(edge,"#c7d0d6",9,46);drawZoneLabel((TUNNEL[0]+TUNNEL[1])/2,53,"TUNNEL","#c7d0d6");
