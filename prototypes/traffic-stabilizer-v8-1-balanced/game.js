...
const TOTAL_LAPS=3,JAM_SCORE_LIMIT=8.5,FAIL_CONFIRM=.80;
const HILL=[52,126],CURVE=[160,222],TUNNEL=[266,326],MERGE_S=236;
const EVENTS=[
 {at:.55*L,label:"ON-RAMP +1",cars:[false]},
 {at:1.16*L,label:"ON-RAMP +1",cars:[false]},
 {at:1.72*L,label:"TRUCK MERGING",cars:[true]},
 {at:2.16*L,label:"RUSH +1",cars:[false]},
 {at:2.58*L,label:"FINAL RUSH +2",cars:[false,true]}
];
...
function reset(){
  cars=[];rampCars=[];const N=19,initial=24/3.6;
...
function updateCongestion(dt){
  ...
  for(let i=0;i<n;i++){
    ...
    const tau=raw>c.jamLevel?3.5:1.3;
