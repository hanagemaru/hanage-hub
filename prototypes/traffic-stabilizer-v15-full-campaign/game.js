...
function showModeSelect(){
  ...
  const best=getEndlessBest(),daily=buildDailyConfig(),dr=getDailyRecord(daily.dateKey),records=getStageRecords();
  const count=ch=>{let n=0;for(let i=(ch-1)*7;i<ch*7;i++)if(records[String(i)]?.cleared)n++;return n};
  overlay.innerHTML=`<div class="overlayCard"><h2>SELECT MODE</h2><div class="modeMenu"><button id="ch1" class="modeBtn">CHAPTER 1<small>${count(1)}/7 CLEAR · BASIC FLOW</small></button><button id="ch2" class="modeBtn">CHAPTER 2<small>${count(2)}/7 CLEAR · ADVANCED FLOW</small></button><button id="ch3" class="modeBtn">CHAPTER 3<small>${count(3)}/7 CLEAR · ROAD WORKS</small></button><button id="ch4" class="modeBtn">CHAPTER 4<small>${count(4)}/7 CLEAR · MASTER FLOW</small></button><button id="daily" class="modeBtn daily">DAILY CHALLENGE<small>${daily.dateKey} · ${daily.dailyLabel} · ${dr.cleared?"CLEAR":"NEW"}</small></button><button id="endless" class="modeBtn endless">ENDLESS FLOW<small>BEST ${best.toFixed(1)} LAPS · GRIDLOCKまで走る</small></button></div></div>`;
  overlay.classList.remove("hidden");
