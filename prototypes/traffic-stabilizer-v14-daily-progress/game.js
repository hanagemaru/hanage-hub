...
function buildDailyConfig(){
  const key=japanDayKey(),rnd=rng32(hashText("traffic-stabilizer-"+key)),type=Math.floor(rnd()*5);
  const c={name:"DAILY CHALLENGE",short:"DAILY",desc:key+" · 同日同条件",n:19+Math.floor(rnd()*3),laps:5,jamLimit:9.0,daily:true,pace:REGULAR_SIM_SPEED,dateKey:key};
  if(type===0){c.hill=true;c.curve=true;c.trucks=1}
  if(type===1){c.curve=true;c.tunnel=true;c.n+=1}
  if(type===2){c.hill=true;c.tunnel=true;c.trucks=1}
  if(type===3){c.hill=true;c.curve=true;c.tunnel=true;c.trucks=1;c.jamLimit=9.3}
  if(type===4){c.hill=true;c.trucks=2}
  const base=.55+rnd()*.25,step=.88+rnd()*.18;
  c.events=[];
  for(let k=0;k<5;k++){
    const isTruck=(k===2&&rnd()>.45);
    c.events.push({at:base+k*step,cars:[isTruck],label:isTruck?"DAILY TRUCK":"DAILY MERGE"});
  }
  return c;
}
...
let stageIndex=0,gameMode="campaign",cars=[],rampCars=[],player=null,started=false,running=false,paused=false,lastTs=0,totalDist=0,nextId=100,eventIndex=0,noticeTimer=0,autoBrake=false,failClock=0,resultShown=false,elapsedReal=0,maxJamPct=0,tutorialStep=0;
...
function reset(i=stageIndex,nextMode=gameMode){
  ...
  if(gameMode==="daily")noteDailyAttempt(s.dateKey);
  if(s.seed){
...
function finish(ok){
  ...
  if(cfg().daily){
    ...
    overlay.innerHTML=ok
      ?`<div class="overlayCard"><h2 class="good">DAILY CLEAR</h2><div class="dailyDate">${key}</div><div class="resultStats"><div class="resultStat"><span>MAX JAM</span><b>${Math.round(maxJamPct)}%</b></div><div class="resultStat"><span>TIME</span><b>${formatTime(elapsedReal)}</b></div></div><div class="dailyScore">BEST ${rec.bestTime!=null?formatTime(rec.bestTime):"--"} · JAM ${rec.bestJam!=null?Math.round(rec.bestJam)+"%":"--"} · ${rec.attempts||1} ATTEMPT(S)</div><div class="resultActions"><button id="retryBtn">RETRY</button><button id="modeBtn">MODE SELECT</button></div></div>`
  ...
  overlay.innerHTML=ok
    ?`<div class="overlayCard"><h2 class="good">FINISH</h2><p>${cfg().name}</p><div class="resultStats"><div class="resultStat"><span>MAX JAM</span><b>${Math.round(maxJamPct)}%</b></div><div class="resultStat"><span>TIME</span><b>${formatTime(elapsedReal)}</b></div></div><button id="nextBtn" class="mainBtn">${last?"MODE SELECT":"NEXT STAGE"}</button></div>`
...
function showModeSelect(){
  ...
  const best=getEndlessBest(),daily=buildDailyConfig(),dr=getDailyRecord(daily.dateKey);
  overlay.innerHTML=`<div class="overlayCard"><h2>SELECT MODE</h2><div class="modeMenu"><button id="ch1" class="modeBtn">CHAPTER 1<small>STAGE 1–7 · BASIC</small></button><button id="ch2" class="modeBtn">CHAPTER 2<small>STAGE 8–14 · ADVANCED</small></button><button id="daily" class="modeBtn daily">DAILY CHALLENGE<small>${daily.dateKey} · ${dr.cleared?"CLEAR":"NEW"} · ${dr.attempts||0} ATTEMPT(S)</small></button><button id="endless" class="modeBtn endless">ENDLESS FLOW<small>BEST ${best.toFixed(1)} LAPS · GRIDLOCKまで走る</small></button></div></div>`;
  overlay.classList.remove("hidden");
  $("ch1").onclick=()=>showChapter(1);$("ch2").onclick=()=>showChapter(2);$("daily").onclick=()=>reset(0,"daily");$("endless").onclick=()=>reset(0,"endless");
}
function showChapter(chapter){
  running=false;const start=chapter===1?0:7,end=start+7;
  overlay.innerHTML=`<div class="overlayCard"><h2>CHAPTER ${chapter}</h2><div class="chapterTitle">${chapter===1?"BASIC FLOW":"ADVANCED FLOW"}</div><div id="stageGrid" class="stageGrid compact"></div><button id="backModes" class="backBtn">← MODE SELECT</button></div>`;
  overlay.classList.remove("hidden");
