...
function rawTrackPoint(t){
  const s=t*L,a=2*Math.PI*(t-.528),ca=Math.cos(a),sa=Math.sin(a),n=3.2;
  const sx=ca>=0?1:-1,sy=sa>=0?1:-1;
  let x=CX+215*sx*Math.pow(Math.abs(ca),2/n)+18*sa+8*Math.sin(2*a);
  let y=CY+170*sy*Math.pow(Math.abs(sa),2/n)+14*Math.sin(2*a);
  if(cfg().curve){
    const u=zoneU(s,CURVE);
    if(u>=0){
      const env=Math.sin(Math.PI*u),k=1-.08*env;
      x=CX+(x-CX)*k;
      y=CY+(y-CY)*k+10*env*Math.sin(2*Math.PI*u);
    }
  }
  if(cfg().hill){
...
function render(pre){
  ...
  document.querySelector(".ringHud")?.classList.toggle("criticalHud",st.jamPct>=82);
